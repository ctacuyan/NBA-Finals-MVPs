DROP TABLE IF EXISTS NBAChampions;
...
SELECT Year, MVP, MIN(PPG) as "Lowest PPG by MVP" FROM NBAChampions GROUP BY MVP ORDER BY MVP ASC LIMIT 1;

SELECT Year, MVP, Max(PPG) as "Highest PPG by MVP" FROM NBAChampions GROUP BY MVP ORDER BY MVP DESC LIMIT 1;
...
SELECT TEAM, MVP, COUNT(DISTINCT MVP) as "No. of Individuals who averaged more than 35 ppg and won MVP" FROM NBAChampions where PPG > 35.0 GROUP BY TEAM;

SELECT COUNT(DISTINCT TEAM) as "Number of different teams that have won an NBA title since 1992" FROM NBAChampions;

SELECT TEAM, COUNT(TEAM) AS "Number of each NBA titles won by each team since 1992" FROM NBAChampions GROUP BY TEAM ORDER BY COUNT(TEAM) DESC;
